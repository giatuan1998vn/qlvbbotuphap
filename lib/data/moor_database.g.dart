...
class $TasksTable extends Tasks with TableInfo<$TasksTable, Task> {
  ...
  Map<String, Variable> entityToSql(TasksCompanion d) {
    ...
    if (d.id.present) {
      map['id'] = Variable(d.id.value);
    }
    if (d.name.present) {
      map['name'] = Variable(d.name.value);
    }
    if (d.dueDate.present) {
      map['due_date'] = Variable(d.dueDate.value);
    }
    if (d.completed.present) {
      map['completed'] = Variable(d.completed.value);
    }
    return map;
  }
  ...
  List<GeneratedColumn> get $columns => throw UnimplementedError();
